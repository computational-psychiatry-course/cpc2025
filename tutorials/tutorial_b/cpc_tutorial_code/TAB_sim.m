...
function [MDPs, gen_data] = TAB_sim(alpha, cr, eta_win, eta_loss, prior_a,omega)   
...
%                     0.3672	0.0943	0.6427];
    ...
    for block=1:NB
        mdp = TAB_gen_model(BlockProbs(block, :), 1, alpha, cr, eta_win, eta_loss, prior_a,omega);
        
        % Deal for all NB*TpB trials
        MDP(1:TpB) = deal(mdp);

        % Run simulation routine
        MDP  = spm_MDP_VB_X_eta2_forgetting_CMG(MDP);
