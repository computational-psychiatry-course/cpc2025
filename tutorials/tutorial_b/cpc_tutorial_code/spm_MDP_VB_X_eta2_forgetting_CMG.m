function [MDP] = spm_MDP_VB_X_eta2_forgetting_CMG(MDP,OPTIONS)
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_X_eta2_forgetting_CMG(MDP(:,i),OPTIONS);
...
try, omega   = MDP(1).omega;   catch, omega   = .2;    end
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X_eta2_forgetting_CMG(mdp);
    ...
    for t = 1:T
        ...
        if isfield(MDP,'a')
            for g = 1:Ng(m)
                ...
                da     = da.*(MDP(m).a{g} > 0);
            if sum(da(2,:))>0 && sum(da(3,:))>0
            error('More than one nonzero entry.')
            end
            if g==1 && t==T %
              if sum(da(2,:))>0 %[this should be in da]
                MDP(m).a{g} = (MDP(m).a{g}-MDP(m).a_0{g})*(1-omega) + MDP(m).a_0{g} + da*eta_win;
              elseif sum(da(3,:))>0  %[this should be in da]
                MDP(m).a{g} = (MDP(m).a{g}-MDP(m).a_0{g})*(1-omega) + MDP(m).a_0{g}+ da*eta_loss;
              else
                MDP(m).a{g} = (MDP(m).a{g}-MDP(m).a_0{g})*(1-omega) + MDP(m).a_0{g} + da*eta; %if outcome is not win or loss, not sure if that's possible
              end
            else
              MDP(m).a{g} = (MDP(m).a{g}-MDP(m).a_0{g})*(1-omega) + MDP(m).a_0{g} + da*eta; %any other factor time point
            end
            end
        end
