...
PRIOR_A  = 0.25; % Specify INFORMATION INSENSITIVITY (PRIOR_A) parameter value
OMEGA = 0; % Specify FORGETTING RATE (OMEGA) parameter value
...
if SIM
    [sim_mdp, gen_data] = TAB_sim(ALPHA, CR, ETA_WIN, ETA_LOSS, PRIOR_A,OMEGA);
...
if FIT
    if SIM
        ...
        if PLOT_FLAG
            figure
            gtitle = sprintf('Fit Model (to %s Subject Data, Alpha=%.2f; CR=%.2f; Eta Win=%.2f; Eta Loss=%.2f; Prior A=%.2f; Omega=%.2f)', FIT_SUBJECT, fit_results{3}(1), fit_results{3}(2), fit_results{3}(3), fit_results{3}(4), fit_results{3}(5),fit_results{3}(6));
            TAB_plot_fit_demo(fit_results{5}(1:16)', fit_results{3}(5), gtitle);
        end
    end

    fprintf('Fit: \n\tAlpha =\t%.3f\n\tCR =\t%.3f\n\tEta Win =\t%.3f\n\tEta Loss =\t%.3f\n\tPrior A =\t%.3f\n\tOmega =\t%.3f\n\tNegative Free Energy =\t%.3f\n', fit_results{3}(1), fit_results{3}(2), fit_results{3}(3), fit_results{3}(4), fit_results{3}(5),fit_results{3}(6),fit_results{1,4}.F);
end
